          ...
          return (
            <div key={item.id} className="relative group">
              ...
              <div className={`
                ...
                ${level === 0 
                  ? 'bg-gradient-to-r from-indigo-600/15 to-transparent border-indigo-500/40 shadow-lg' 
                  : 'bg-[#1a1a1a] border-white/10 hover:border-indigo-500/30'}
                ${isEditing ? 'ring-2 ring-yellow-500 border-yellow-500 bg-yellow-500/5' : ''}
              `}>
                <div className="flex items-center gap-4 flex-1">
                  {/* Số thứ tự */}
                  <div className={`
                    w-10 h-10 rounded-xl flex items-center justify-center text-[10px] font-black shrink-0 shadow-inner
                  ...
                  <div className="flex-1">
                    {isEditing ? (
                      <div className="space-y-2">
                        <input 
                            type="text" 
                            className="w-full bg-[#0d0d0d] border border-white/10 rounded-lg p-2 text-xs text-white outline-none focus:border-yellow-500"
                        ...
                        <input 
                            ...
                            className="w-full bg-[#0d0d0d] border border-white/10 rounded-lg p-2 text-[10px] text-gray-500 outline-none"
                      ...
                      <>
                        <h5 className={`tracking-tight ${level === 0 ? 'text-base font-black text-white' : 'text-sm font-bold text-gray-300'}`}>
                          {item.title}
                        </h5>
                        {item.url && (
                          <div className="flex items-center gap-1 mt-1 text-indigo-400/40 text-[9px] font-mono">
                ...
                <div className="flex items-center gap-2 ml-4">
                  {isEditing ? (
                    ...
                    <>
                      {/* Nút THÊM CON: Chỉ cho phép thêm 1 cấp sâu hơn cho mục này */}
                      <button 
                        onClick={() => {
                            const isOpening = addingToId !== item.id;
                            setAddingToId(isOpening ? item.id : null);
                            if (isOpening) {
                                // TỰ ĐỘNG LẤY STT TIẾP THEO
                                const existingChildren = book.chapters.filter(c => c.parentId === item.id);
                                setNewChildItem({ title: '', url: '', stt: existingChildren.length + 1 });
                            }
                        }}
              ...
              {/* Form Thêm Bài mới (Hiện ngay bên dưới cha) */}
              {addingToId === item.id && (
                <div className="ml-10 mt-3 p-5 bg-indigo-600/5 border border-indigo-500/30 rounded-2xl animate-slide-up shadow-2xl relative z-10">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                      <div className="md:col-span-5">
                        ...
                        <input 
                          type="text" placeholder="Nhập tên bài..." 
                      ...
                      <div className="md:col-span-4">
                        ...
                        <input 
                          type="text" placeholder="Link PDF..." 
              ...
              {/* Gọi đệ quy cho các mục con sâu hơn */}
                    ...
                    <div className="max-w-4xl mx-auto space-y-12">
                      ...
                      <div className="bg-indigo-600/5 p-8 rounded-[2rem] border border-indigo-500/20">
                         <h5 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                           <ListPlus size={18} /> Bước 1: Tạo các Chương Chính (Cấp 1)
                         </h5>
                         <div className="flex gap-4">
                            <input 
                              type="text" placeholder="VD: Chương 1: Giới thiệu..." 
                              value={newRootTitle}
                              onChange={e => setNewRootTitle(e.target.value)}
                              className="flex-1 bg-black border border-white/10 rounded-xl p-4 text-sm text-white focus:border-indigo-500 outline-none shadow-inner"
                            />
                      ...
                      <div className="space-y-8">
                         <h5 className="text-[12px] font-black text-white uppercase tracking-[0.5em] flex items-center gap-4">
                            <ListTree size={20} className="text-indigo-500" /> Sơ đồ cây đa tầng
                            <div className="h-px flex-1 bg-white/5"></div>
                         </h5>
                         <div className="bg-[#050505] p-10 rounded-[3.5rem] border border-white/5 min-h-[400px] shadow-inner relative overflow-hidden">
                            {/* Hiệu ứng Background Lưới */}
                            ...
                            <div className="relative z-10">
                                {book.chapters.length === 0 ? (
                                    <div className="text-center py-40 flex flex-col items-center justify-center">
                                        <div className="w-20 h-20 bg-gray-900 rounded-full flex items-center justify-center text-gray-800 border border-white/5 mb-8">
                                            <Minus size={32} />
                                        </div>
                                        <p className="text-gray-700 font-black uppercase tracking-[0.4em] text-[12px]">Chưa có dữ liệu mục lục</p>
                                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 animate-slide-up">
            <div className="lg:col-span-1">
                <div className="bg-[#222] p-8 rounded-[2.5rem] border border-white/5 shadow-2xl sticky top-8">
                    ...
                    {/* Fix: Changed onSubmit from handleUpdateChapter to handleAddCategory */}
                    <form onSubmit={handleAddCategory} className="space-y-6">
                        ...
                        {/* Fix: Removed redundant onClick manual call to handleAddCategory */}
