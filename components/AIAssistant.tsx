...
const AIAssistant: React.FC<AIAssistantProps> = ({ isVisible, onClose, pageText }) => {
       ...
       setMessages([
        {
          id: 'intro',
          role: 'model',
          text: 'Xin chào! Tôi là trợ lý AI Gemini. Tôi có thể giúp tóm tắt trang sách hoặc phân tích tài liệu bên ngoài.',
          timestamp: Date.now()
        }
      ]);
    }
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-[380px] bg-slate-900 border-l border-white/5 shadow-2xl z-50 flex flex-col transform transition-transform duration-300 ease-in-out">
      <div className="bg-slate-800/80 backdrop-blur-xl border-b border-white/5">
          <div className="p-4 flex justify-between items-center">
            <div className="flex items-center gap-2 text-indigo-400">
                <Sparkles size={16} />
                <h2 className="font-black text-xs uppercase tracking-widest">Gemini Assistant</h2>
            </div>
            <button onClick={onClose} className="text-slate-500 hover:text-white transition-colors"><X size={18} /></button>
          </div>
          <div className="flex px-3 pb-3 gap-1.5">
              <button onClick={() => setMode('reader')} className={`flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${mode === 'reader' ? 'bg-indigo-600 text-white' : 'bg-slate-700/50 text-slate-400 hover:bg-slate-700'}`}><BookOpen size={12} /> Trang Sách</button>
              <button onClick={() => setMode('notebook')} className={`flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${mode === 'notebook' ? 'bg-indigo-600 text-white' : 'bg-slate-700/50 text-slate-400 hover:bg-slate-700'}`}><Library size={12} /> Notebook</button>
          </div>
      </div>
          ...
          <div className="bg-slate-800/30 border-b border-white/5 p-3">
              ...
              {!showAddSource ? (
                  ...
                  <div className="bg-slate-950 p-3 rounded-xl border border-white/10 animate-slide-up">
                      ...
                      {newSourceType === 'text' ? (
                          <textarea value={newSourceContent} onChange={(e) => setNewSourceContent(e.target.value)} placeholder="Nội dung văn bản..." className="w-full bg-slate-900 border border-white/5 rounded-lg p-2.5 text-xs text-white min-h-[70px] mb-2 focus:border-indigo-500 outline-none" />
                      ) : (
                          <input type="text" value={newSourceContent} onChange={(e) => setNewSourceContent(e.target.value)} placeholder="https://..." className="w-full bg-slate-900 border border-white/5 rounded-lg p-2.5 text-xs text-white mb-2 focus:border-indigo-500 outline-none" />
                      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-900/50">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[88%] rounded-xl p-3 text-[13px] leading-relaxed ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none shadow-lg' : 'bg-slate-800 text-slate-200 rounded-bl-none border border-white/5'}`}>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-slate-800 rounded-xl p-3 rounded-bl-none flex items-center gap-2 text-slate-400 border border-white/5">
              <Loader2 size={12} className="animate-spin text-indigo-400" />
              <span className="text-[10px] font-medium italic opacity-70">Gemini đang phản hồi...</span>
            </div>
          </div>
        )}
      ...
      <div className="p-4 bg-slate-800 border-t border-white/5 shadow-2xl">
        <div className="relative">
          <textarea
            ...
            className="w-full bg-slate-900 border border-white/10 rounded-xl pl-3 pr-10 py-3 text-[13px] text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 resize-none h-11"
          />
          <button onClick={handleSend} disabled={isLoading || !input.trim()} className="absolute right-2 top-2 p-1.5 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-white disabled:opacity-30 disabled:grayscale transition-all shadow-lg shadow-indigo-600/20"><Send size={14} /></button>
        </div>
      </div>
    </div>
  );
