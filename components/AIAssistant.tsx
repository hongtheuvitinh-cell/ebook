...
const AIAssistant: React.FC<AIAssistantProps> = ({ isVisible, onClose, pageText }) => {
      ...
      if (window.aistudio) {
          try {
              ...
              setMessages(prev => [...prev, {
                  ...
                  role: 'model',
                  text: "Đã cập nhật cấu hình API Key. Bây giờ bạn có thể tiếp tục trò chuyện!",
                  timestamp: Date.now()
              }]);
          } catch (e) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (responseText === "ERROR_API_KEY") {
          setApiError(true);
          responseText = "Xin lỗi, tôi không thể kết nối. Hệ thống yêu cầu bạn phải chọn một API Key hợp lệ (Paid Project) để sử dụng tính năng này.";
      }
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        role: 'model',
        text: "Lỗi kết nối nghiêm trọng. Vui lòng kiểm tra API Key của bạn bằng nút bên dưới.",
        timestamp: Date.now()
      }]);
    } finally {
      setIsLoading(false);
  ...
  const handleQuickAction = async (action: 'summarize' | 'explain') => {
    ...
    try {
      ...
      if (responseText === "ERROR_API_KEY") {
          setApiError(true);
          setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: 'model', text: "Yêu cầu API Key để tiếp tục.", timestamp: Date.now() }]);
      } else {
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-[420px] bg-slate-900 border-l border-white/5 shadow-2xl z-50 flex flex-col transform transition-transform duration-300 ease-in-out">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 bg-slate-900/50 custom-scrollbar">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] rounded-2xl p-4 text-[14px] leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none border border-white/5'}`}>
        ...
        {apiError && (
            <div className="bg-rose-950/30 border border-rose-500/20 p-5 rounded-2xl flex flex-col items-center text-center gap-4 animate-slide-up">
                <div className="p-3 bg-rose-500/20 rounded-full text-rose-400">
                    <AlertTriangle size={28} />
                </div>
                <div>
                    <h3 className="text-white font-bold text-sm mb-1">Yêu cầu chọn API Key</h3>
                    <p className="text-[12px] text-rose-200/70 font-medium">Bạn cần chọn một dự án Google Cloud có tính phí để sử dụng tính năng này.</p>
                </div>
                <button 
                    onClick={handleOpenKeySelector}
                    className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-600/20 transition-all active:scale-95"
                >
                    <Key size={14} /> Mở bảng chọn API Key
                </button>
            </div>
        )}
